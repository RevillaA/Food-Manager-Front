...
import {
	DailySession,
	DailySessionStatus,
} from "../models/daily-session.interface";
...
export class DailySessionsService {
	...
	openDailySession(
		payload: OpenDailySessionRequest,
	): Observable<DailySessionResponse> {
		return this.http.post<DailySessionResponse>(
			`${this.baseUrl}/open`,
			payload,
		);
	}
	...
	closeDailySession(
		...
		return this.http.patch<DailySessionResponse>(
