...
export class SalesList implements OnInit {
	...
	readonly paidTodayCount = computed(() => {
		return this.todaySales().filter((sale) => sale.payment_status === "PAID")
			.length;
	});

	readonly pendingTodayCount = computed(() => {
		return this.todaySales().filter((sale) => sale.payment_status === "PENDING")
			.length;
	});
	...
	loadSalesOfToday(): void {
		...
		this.salesService
			.getSalesOfDay(1, 100, { date: this.getTodayString() })
			.subscribe({
				next: (response) => {
