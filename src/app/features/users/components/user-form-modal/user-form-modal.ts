...
export class UserFormModal {
	...
	constructor() {
		effect(
			() => {
				...
				// Se leen explícitamente para que el effect reaccione a esos cambios
				void currentMode;
				void currentUser;
	...
	submit(): void {
		...
		if (this.mode() === "create") {
			const payload: CreateUserRequest = {
				...
				email: this.normalizeOptionalText(
	...
	private resolveErrorMessage(error: unknown): string {
		...
		if (httpError.status === 400) {
			return (
				httpError.error?.message || "Datos inválidos. Revisa el formulario."
			);
		}
