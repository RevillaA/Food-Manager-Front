...
export class ReportList implements OnInit {
	...
	private loadInitialReports(): void {
		...
		this.dailySessionsService.getActiveDailySession().subscribe({
			next: (response) => {
				const sessionDate =
					this.normalizeDateValue(response.data.session_date) || fallbackDate;
		...
		this.dailySessionsService.getDailySessions(1, 1).subscribe({
			next: (response) => {
				const latestSessionDate =
					this.normalizeDateValue(response.data[0]?.session_date) ||
					fallbackDate;
	...
	loadCategorySummary(dateFrom: string, dateTo: string): void {
		...
		this.reportsService.getCategorySummaryReport(dateFrom, dateTo).subscribe({
			...
			error: (error: HttpErrorResponse) => {
				...
				this.errorMessage.set(
					error.error?.message ||
						"No se pudo cargar el resumen por categorías.",
				);
			},
		});
	}
	...
	printDailyReport(): void {
		...
		if (!report) {
			this.errorMessage.set(
				"Primero carga el reporte diario para poder imprimirlo.",
			);
			return;
		}
	...
	printConsolidatedReport(): void {
		...
		if (!rangeReport) {
			this.errorMessage.set(
				"Primero carga el reporte por rango para poder imprimirlo.",
			);
			return;
		}
