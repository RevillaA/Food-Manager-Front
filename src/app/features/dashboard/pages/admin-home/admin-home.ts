...
export class AdminHome implements OnInit {
	...
	readonly paidSalesCount = computed(() => {
		return this.salesOfToday().filter((sale) => sale.payment_status === "PAID")
			.length;
	});

	readonly pendingSalesCount = computed(() => {
		return this.salesOfToday().filter(
	...
	readonly pendingPaymentOrdersCount = computed(
		() => this.closedOrdersPendingPayment().length,
	);
		...
		this.dailySessionsService.getActiveDailySession().subscribe({
			...
			error: (error: HttpErrorResponse) => {
				...
				this.errorMessage.set(
					error.error?.message ||
						"No se pudo cargar el inicio del administrador.",
				);
			},
		});
	}
		...
		this.ordersService
			...
			.subscribe({
				next: (ordersResponse: OrdersListResponse) => {
					this.salesService.getSalesOfToday().subscribe({
						...
						error: (error: HttpErrorResponse) => {
							...
							this.errorMessage.set(
								error.error?.message ||
									"No se pudieron cargar las ventas del dÃ­a.",
							);
						},
					});
				},
				error: (error: HttpErrorResponse) => {
					...
					this.errorMessage.set(
						error.error?.message ||
							"No se pudieron cargar los pedidos de la jornada.",
					);
				},
			});
	}
