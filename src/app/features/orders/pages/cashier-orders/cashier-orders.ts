...
export class CashierOrders implements OnInit {
	...
	readonly canChargeOrder = computed(() => {
		...
		const isChargedInBoard =
			!!order && this.paidOrders().some((item) => item.id === order.id);
		...
		this.ordersService
			...
			.subscribe({
				...
				error: (error: HttpErrorResponse) => {
					...
					this.errorMessage.set(
						error.error?.message ||
							"No se pudieron cargar los pedidos de la jornada activa.",
					);
				},
			});
	}
	...
	loadSelectedOrder(orderId: string): void {
		...
		this.ordersService.getOrderById(orderId).subscribe({
			next: (response) => {
				if (
					requestId !== this.latestOrderDetailRequestId ||
	...
	addProductToOrder(product: Product): void {
		...
		if (!currentOrder || currentOrder.status !== "OPEN") {
			this.modalErrorMessage.set(
				"Solo puedes agregar productos a pedidos abiertos.",
			);
			return;
		}
